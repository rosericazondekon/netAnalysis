...
V(mnet2)$communitySBM <- communitySBM
...
save(mod1, file = 'modDataFinal/mod1.rda')
...
#' (uniform homophily) for our categorical variable "communitySBM".
...
save(mod2, file = 'modDataFinal/mod2.rda')
...
{{round(mod2$coef[4],2)}}
#' , the homogeneous probability of two authors collaborating between the same community is 
{{plogis(mod2$coef[5])}}
#' , and the homogeneous probability of two authors with the same collaboration type collaborating is 
{{plogis(mod2$coef[6])}}
#' .
#' The log-odds of a collaboration that is homogeneous in all 5 attributes is 
{{sum(mod2$coef)}}
#'  and the associated probability is 
{{plogis(sum(mod2$coef))}}
#' 
#' All the probabilities for each attribute are:
{{plogis(mod2$coef)}}
#' 
...
mod3.f <- formula(mnet3.simple ~ edges + nodecov("timesCited") + nodecov('degree') + nodecov("numPub") + nodematch("communitySBM") + nodematch("collabType") + nodefactor("collabType"))
...
save(mod3, file = 'modDataFinal/mod3.rda')
...
save(sim1, file = 'modDataFinal/sim1.rda')
...
M2 <- mixingmatrix(sim1, "communitySBM")
...
mod4.f <- formula(mnet3.simple ~ edges + triangle + twopath)
t0 <- Sys.time()
mod4 <- ergm(mod4.f, maxit=1000, verbose = FALSE,
...
save(mod4, file = 'modDataFinal/mod4.rda')
#'
#+ mod4-mcmc_diag,cache=FALSE
mcmc.mod4 <- mcmc.diagnostics(mod4)
mcmc.mod4
save(mcmc.mod4, file = "modDataFinal/mcmc-mod4.rda")
#'
#+ mod4-mcmc_diag_save
pdf("./figureFinal/mod4diagnostics.pdf")
mcmc.diagnostics(mod4)
dev.off()

#' We now fit model 3 with the triangle ergm term
#+ formula5,cache=FALSE
mod5.f <- formula(mnet3.simple ~ edges + triangle + twopath + nodecov("timesCited") + nodecov('degree') +
                         nodecov("numPub") + nodematch("communitySBM") + nodematch("collabType") + nodefactor("collabType"))
summary.statistics(mnet3.ergm2)
#'
#+ model-fitting5,cache=FALSE
t0 <- Sys.time()
...
anova.ergm(mod5)
#'
#+ mod5-mcmc_diag,cache=FALSE
mcmc.mod5 <- mcmc.diagnostics(mod5)
mcmc.mod5
save(mcmc.mod5, file = "modDataFinal/mcmc-mod5.rda")
#+ mod5-mcmc_diag_save
pdf("./figureFinal/mod5diagnostics.pdf")
mcmc.diagnostics(mod5)
dev.off()
#'
#+ save-mod5,cache=FALSE
save(mod5, file = 'modDataFinal/mod5.rda')

#' The new model, model 5 converges after 
{{mod3$iterations}}
#'  iterations. We can see that all terms are significant a 
#' model likelihood of 
{{mod3$mle.lik}}
...
#+ simulation2,cache=FALSE
sim2 <- simulate(mod5, 
                 # burnin = 1e+6, # we may set the number of steps in the simulation chain to 1e+06
                 verbose = FALSE, seed = 25)
#'
#+ save-simulation2,cache=FALSE
save(sim2, file = 'modDataFinal/sim2.rda')

#' Let's examine the mixing matrix for the attribute collabType

#+ mixing-matrix2,cache=FALSE
M3 <- mixingmatrix(sim2, "collabType")
confusionMatrix(M3$matrix)
#'
#' Let's examine the mixing matrix for the attribute community
#' #+ mixing-matrix2b,cache=FALSE
M4 <- mixingmatrix(sim3, "communitySBM")
...
mod6.f <- formula(mnet3.simple ~ edges + gwesp(0, fixed = TRUE) + nodecov("timesCited") + twopath
                  + nodecov('degree') + nodecov("numPub") + nodematch("communitySBM") 
                  + nodematch("collabType") + nodefactor("collabType"))
t0 <- Sys.time()
mod6 <- ergm(mod6.f, verbose = FALSE, control = control.ergm(MCMLE.maxit=1000, MCMC.samplesize = 10000,
                                                            MCMC.interval = 10000, seed=25))
Sys.time() - t0
summary(mod6)
anova.ergm(mod6)
#'
#+ save-mod6,cache=FALSE
save(mod6, file = 'modDataFinal/mod6.rda')
#'
#+ mod6-mcmc_diag,cache=FALSE
mcmc.mod6 <- mcmc.diagnostics(mod6)
mcmc.mod6
save(mcmc.mod6, file = "modDataFinal/mcmc-mod6.rda")
#'
#+ mod6-mcmc_diag_save,cache=FALSE
pdf("./figureFinal/mod6diagnostics.pdf")
...
#'
#+ mod6-deg,cache=FALSE
gof6.deg <- gof(mod6 ~ degree, verbose = FALSE)
plot(gof6.deg)
#'
# + gof4,cache=FALSE
# gof4.deg
#'
#+ gof6-deg,cache=FALSE
pdf("./figureFinal/gof6-deg.pdf")
plot(gof6.deg)
dev.off()

#' Let us now compare our model fit to our data on degree distribution
#+ gof-mod6,cache=FALSE
t0 <- Sys.time()
diag.mod6 <- gof(mod6, control=control.gof.ergm(parallel=cores, parallel.type="PSOCK"))
Sys.time() - t0
jpeg(filename = './figureFinal/gof-mod6.jpg')
plot(diag.mod6)
dev.off()
mcmc.diagnostics(mod6)
#'
#+ save-diag-mod6,cache=FALSE
save(diag.mod6, file = "modDataFinal/diag.mod6.rda")
