...
net2 <- list(net1, net2, net3, net4, net5, net6, net7)
...
while(i<=length(net2)){
  ...
  v.attrs$collabType <- V(nt)$collab
...
#                      nodematch("community") + nodematch("collab") + nodefactor("collabType") + 
#                      memory(type = "autoregression") + timecov(transform = function(t) (1 + t + t^2))
#                    , R = 1000, ncpus = cores, cl=cl, iterations=1000,control = control.ergm(MCMLE.maxit = 1000,
#                                                                                             MCMC.burnin = 20000))
# Sys.time() - t0
...
texreg(list(model.2d, model.2e, model.2f, model.2g), single.row = TRUE,include.nobs = FALSE, 
       file = "tergm_sub2/malaria-coauthNet5.tex",
...
#########################################
list.malnet <- list.net2
t0 <- Sys.time()
maltergm <- mtergm(list.malnet ~ edges + nodecov("timesCited") + nodecov('degree') + nodecov("numPub") +
                     nodematch("community") + nodematch("collab") + nodefactor("collabType") + 
                     memory(type = "stability") + timecov(transform = function(t) t)
                   , R = 1000, ncpus = cores, cl=cl, iterations=1000,control = control.ergm(MCMLE.maxit = 1000,
                                                                                            MCMC.burnin = 20000)) # delrecip does not have any effct on mdel improvement
...
#Save
saveRDS(maltergm,"tergm_sub2/maltergm.rds")

saveRDS(list.malnet,"tergm_sub2/listmalnet.rds")
