...
t0 <- Sys.time()
mod1 <- ergm(my.ergm.bern, iterations=1000, 
...
mnet3.ergm2 <- formula(mnet3.simple ~ edges + nodecov("timesCited") + nodecov('degree') +
...
t0 <- Sys.time()
mod3 <- ergm(mnet3.ergm2, iterations=1000,
                       control=control.ergm(parallel=cores, parallel.type="PSOCK", MCMC.samplesize=1000,
                                            MCMC.interval=5000, MCMC.burnin = 20000, seed=25, MCMLE.maxit = 1000))
...
summary(mod3)
anova.ergm(mod3)
save(mod3, file = 'modData/mod3.rda')
...
t0 <- Sys.time()
mod4 <- ergm(mnet3.simple ~ edges + triangle, maxit=1000, verbose = TRUE,
...
mnet3.ergm2 <- formula(mnet3.simple ~ edges + triangle + nodecov("timesCited") + nodecov('degree') +
                         nodecov("numPub") + nodematch("community") + nodematch("collabType") + nodefactor("collabType"))
summary.statistics(mnet3.ergm2)

#+ model-fitting5,cache=FALSE
t0 <- Sys.time()
mod5 <- ergm(mnet3.ergm2, iterations=1000,
             control=control.ergm(parallel=cores, parallel.type="PSOCK", MCMC.samplesize=1000,
                                  MCMC.interval=5000, MCMC.burnin = 20000, seed=25, MCMLE.maxit = 1000))
#save(mod2, file = './Rdata/mod2.rda')
Sys.time() - t0
