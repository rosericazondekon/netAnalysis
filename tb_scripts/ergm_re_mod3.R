...
mnet<-get(load('./Rdata/TBnet.rda'))
mnet2<-get(load('./Rdata/TBnet2.rda'))
load('./Rdata/TBnet2.gc.rda')
# load('./Rdata/auth_data.rda')
# load('./Rdata/edges.rda')
# load('./Rdata/mnet2.gc.rda')
communitySBM<-get(load('./Rdata/communitySBM.rda'))

# /* Loading a simplified version of the network */
mnet3 <- read_graph('./graphs/TBnet.graphml', format = 'graphml')
mnet.w <- read_graph('./graphs/TBnet_weighted.graphml', format = 'graphml')
...
save(collabType,file = "./Rdata/collabType.rda")
...
#'
#+ preproc,cache=FALSE
A <- get.adjacency(mnet2)
# v.attrs <- get.data.frame(mnet2, what="vertices")
...
network::set.vertex.attribute(mnet3.simple, "communitySBM", communitySBM)
...
model1.f <- formula(mnet3.simple ~ edges)
model1.f
...
save(model1, file = 'ergm/smodel1.rda')
...
save(model2, file = './ergm/smodel2.rda')
...
#'
#+ formula3
model3.f <- formula(mnet3.simple ~ edges + nodecov("timesCited") + nodecov('degree') +
...
model3 <- ergm(model3.f, iterations=1000, 
               control=control.ergm(parallel=cores, parallel.type="PSOCK", MCMC.samplesize=1000,
                                    MCMC.interval=50, MCMLE.maxit = 100, MCMC.burnin = 2000, seed=25))
Sys.time() - t0
...
# mcmc.model3
# save(mcmc.model3, file = "ergm/mcmc-smodel3.rda")
#'
#+ save-mod3,cache=FALSE
save(model3, file = 'ergm/smodel3.rda')
...
#'
#+ formula4
model4.f <- formula(mnet3.simple ~ edges + triangle)
model4.f
...
Sys.time() - t0
save(diag.model3, file = 'ergm/diag.smodel3.rda')
jpeg(filename = './ergm/gof-smodel3.jpg')
plot(diag.model3)
dev.off()
...
texreg(list(model1, model2, model3), single.row = TRUE,include.nobs = FALSE, 
       file = "ergm/ergm_tb-coauthNet5.tex",
       caption = "ERGM of the TB Co-authorship Network.",label = "tab:tergm"
       , custom.model.names =c("Model~1", "Model~2", "Model~3"), use.packages = FALSE,
