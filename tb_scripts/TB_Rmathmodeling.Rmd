...
This section follows from the first section dedicated to the Descriptive Analysis of our co-authorship network (available [HERE](http://rpubs.com/rosericazondekon/TB_descstat)).

The purpose of this section is the use of mathematical models to describe our tuberculosis co-authorship network.
...
```{r}
nv <- vcount(tbnet2) # number of vertices in tbnet2
ne <- ecount(tbnet2) # number of edges in tbnet2
degs <- degree(tbnet2) # degree distribution in tbnet2
...
r <- mean(neighborhood.size(tbnet2, 1)) # Estimated average neighborhood size
...
Let's recall that our hierarchical clustering algorithm detected 6 communities in our co-authorship network and 9 in the giant component. 
<!-- The graph above clearly demonstrates that the number of communities detected would be considered unusual from the perspective of both Classical random graphs and generalized random graphs. -->
...
```{r}
t.test(num.comm.rg, mu=6)
t.test(num.comm.grg, mu=6)
```
As you can see from the output, p-value < 0.0001 and the 95% CIs for both random graph models do not contain 6. We therefore have over 99% certitude that the observed number of communities is extreme per the random graph models.
...
```{r}
summary(av.clus.ws)
t.test(av.clus.ws, mu=transitivity(tbnet2))

```
```{r}
summary(av.clus.ba)
```
Recall the clustering coefficient for our network is `r I(transitivity(tbnet2))`

Supprisingly, there is substantially more clustering in our tuberculosis co-authorship network than expected from both the random graph models and the two small-world models (t.test displays p-value < 0.0001).


Summarizing the resulting distribution of average path length:
```{r}
summary(av.path.len.rg)
t.test(av.path.len.rg, mu=average.path.length(tbnet2))
...
```
Recall the clustering coefficient for our network is `r I(average.path.length(tbnet2))`
...
**CONCLUSION:** This analysis disproves the evidence for small-world behavior suspected in our previous tutorial. Our observed tuberculosis co-authorship network has some behaviors that the mathematical graph models explored in this tutorial cannot explain. We therefore suggest the application of other models such as Statistical models for Network Graphs to better explain the observed tuberculosis co-authorship network. This will be the focus of our next tutorial.
$\\$
...
```{r}
save(results, file = './Rdata/TBresults.rda')
```
$\\$
