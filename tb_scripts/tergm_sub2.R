...
n1<-read_graph('./graphs/sub2/CAnet_1996-2008.graphml', format = 'graphml')
E(n1)$weight <- 1
...
n2<-read_graph('./graphs/sub2/CAnet_2009-2011.graphml', format = 'graphml')
E(n2)$weight <- 1
net2 <- simplify(n2, 
...
n3<-read_graph('./graphs/sub2/CAnet_2012-2013.graphml', format = 'graphml')
E(n3)$weight <- 1
net3 <- simplify(n3, 
                 edge.attr.comb = list(weight="sum",timesCited="sum",numPub="sum",
                                       key="concat",subject="concat",year="concat",wosid="concat",
...
n4<-read_graph('./graphs/sub2/CAnet_2014-2015.graphml', format = 'graphml')
E(n4)$weight <- 1
net4 <- simplify(n4, 
                 edge.attr.comb = list(weight="sum",timesCited="sum",numPub="sum",
                                       key="concat",subject="concat",year="concat",wosid="concat",
...
n5<-read_graph('./graphs/sub2/CAnet_2016.graphml', format = 'graphml')
E(n5)$weight <- 1
net5 <- simplify(n5, 
                 edge.attr.comb = list(weight="sum",timesCited="sum",numPub="sum",
                                       key="concat",subject="concat",year="concat",wosid="concat",
...
net2 <- list(net1, net2, net3, net4, net5)
...
texreg(list(model.2d, model.2e, model.2f, model.2g, model.2h, model.2hb, model.2hc), single.row = TRUE,include.nobs = FALSE, 
       ...
       caption = "Temporal ERGM of TB Co-authorship Network.",label = "tab:tb_tergm"
...
# Model diagnostic 2a
t0 <- Sys.time()
gof.mod2g <- gof(model.2g, nsim = 200, target = list.net2[5],
                 formula = list.net2[1:4] ~ edges + nodecov("timesCited") + nodecov('degree') + nodecov("numPub") +
                   nodematch("community") + nodematch("collabType") + nodefactor("collabType") +
                   memory(type = "stability") + timecov(transform = function(t) t),
                 ...
                 parallel ="multicore", ncpus = cores, cl=cl)
Sys.time() - t0
save(gof.mod2g, file = "tergm_sub2/diag_mod2g.rda")
...
#######################################################
list.tbnet <- list.net2
t0 <- Sys.time()
tbtergm <- mtergm(list.tbnet ~ edges + nodecov("timesCited") + nodecov('degree') + nodecov("numPub") +
                     nodematch("community") + nodematch("collab") + nodefactor("collabType") + 
                     memory(type = "stability") + timecov(transform = function(t) t)
                   , R = 1000, ncpus = cores, cl=cl, iterations=1000,control = control.ergm(MCMLE.maxit = 1000,
                                                                                            MCMC.burnin = 20000)) # delrecip does not have any effct on mdel improvement
...
#Saving tergm model...
saveRDS(tbtergm,"tergm_sub2/tbtergm.rds")

saveRDS(list.tbnet,"tergm_sub2/listtbnet.rds")
